...
class CommonStepDefs(
    ...
    private val testUserMapping =
        mapOf<String, String>("ntfusr1" to "secret", "ntfusr2" to "secret", "ntfapvr" to "secret")

    private val logger = LoggerFactory.getLogger(CommonStepDefs::class.java)

    companion object {
        const val localhost = "http://localhost:8080/api/v1"
        private const val baseURL = localhost
    ...
    init {
        ...
        When("we call GraphQL with requestBody") { body: String ->
            ...
            set(post(user(), "$localhost/graphql", request, mapOf("request-type" to "entity")))
        }
